  ...
  setup(props, { attrs, emit, slots }) {
    ...
    const getComponentsProps = useComputed((schema) => {
      ...
      if (isFunction(componentProps)) {
        componentProps = componentProps(state.value) ?? {}
      }
    ...
    const getShow = useComputed((schema) => {
      ...
      if (isFunction(show)) {
        isShow = show(unref(state))
      }
      if (isFunction(ifShow)) {
        isIfShow = ifShow(unref(state))
      }
    ...
    const getDisable = useComputed((schema) => {
      ...
      if (isFunction(dynamicDisabled)) {
        disabled = dynamicDisabled(unref(state))
      }
      return disabled
    })
    ...
    const renderFormItem = (schema) => {
      ...
      if (component === 'Divider') {
        ...
        const getContent = (schema) => {
          return slot
            ? getSlot(slots, slot, getValues(schema))
            : render
              ? render(getValues(schema))
    ...
    const renderComponent = (schema) => {
      const { renderComponentContent, component, field, label } = schema
